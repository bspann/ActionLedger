...
Desktop web, single surface, Blazor WebAssembly current stable with routable `@page` components and scoped services, MudBlazor. `DESIGN.md` is the visual identity reference and names the four semantic color families the product adds to the prebuilt theme. This spine specifies behavior only. Vocabulary is the PRD Glossary, used verbatim.

The PRD addendum's front-end layering rule (no HTTP calls in components, typed clients as the model) is architecture, not experience. It appears here only as the Feature folder column below. Shared client state — the signed-in User and the User directory the owner controls select from — lives in the scoped services `Core/Auth/SessionState.cs` and `Core/Users/UserDirectory.cs`, not in components. `docs/frontend-architecture.md` will diagram the mapping at tag time.

Wireframe level. Layout and behavior are decided here. Visual polish beyond MudBlazor defaults is a non-goal.
...
|---|---|---|---|---|---|
| Login | `/login` | `Features/Auth` | Unauthenticated redirect | Username and password, JWT into memory | FR-23 |
| Meeting List | `/meetings` | `Features/Meetings` | Toolbar "Meetings", app open | Meetings newest first with run and action counts; "New meeting" button | FR-3 |
| Meeting Detail | `/meetings/{id}` | `Features/Meetings` | Meeting List row | Fields, Meeting Notes (paste once), Extraction Runs list, "Run extraction" | FR-1, FR-2, FR-3, FR-4 |
| Run Detail | `/meetings/{id}/runs/{runId}` | `Features/Review` | Meeting Detail run row | Run metadata, warnings, failure reason, Proposed Actions with Review States and decisions, "Review" and "Run again" | FR-6, FR-9 |
| Review Screen | `/meetings/{id}/runs/{runId}/review` | `Features/Review` | Run Detail "Review", Meeting Detail "Review" on the latest Succeeded run | Two-pane: Meeting Notes beside proposal cards; Approve, Edit, Reject | FR-10 to FR-15 |
| Action List | `/actions` | `Features/Actions` | Toolbar "Actions", "View actions" links | Filterable, sortable, paged table of Tracked Actions with the Overdue indicator | FR-18, FR-19 |
| Action Detail | `/actions/{id}` | `Features/Actions`, `Features/Audit` | Action List row, "View action" on a decided card | Fields, status control, edit, and the Audit Trail | FR-17, FR-20, FR-22 |

Each surface is a routable component under its feature folder — `Features/<Feature>/<Noun>Page.razor` carrying an `@page` directive — and reaches the API through `Features/<Feature>/Data/<Feature>Service.cs`. The unauthenticated redirect is a route-level auth check, not a router guard.

Closure: every stated need lands on a surface. Paste notes and extract on Meeting Detail, review on Review Screen, see overdue on Action List, prove provenance on Action Detail, log in on Login. Every surface is reached by a Key Flow below. The webhook receiver and Swagger UI are separate pages outside the Blazor app and are not designed here.
...
Behavioral. Visual specs live in `DESIGN.md.Components` or in MudBlazor defaults. MudBlazor ships the Material Symbols icon set, so every icon name `DESIGN.md` specifies — `auto_awesome`, `person`, `warning`, `schedule`, `edit` — carries over unchanged and renders through `MudIcon`.
...
| Toolbar | Global | A `MudAppBar`. Product name links to Meeting List. "Meetings" and "Actions" links show active state. A `MudMenu` user menu shows display name and Role, one item: "Sign out". |
| Meeting table | Meeting List | A `MudTable`; sorting and pagination are built in. Columns: Title, Date, Runs, Tracked Actions. Row click opens Meeting Detail. Sort by Meeting date descending, then creation time descending. The paginator appears at 50 rows. |
| New meeting dialog | Meeting List | A `MudDialog` opened through `IDialogService`. Title (required, max 200), Date (required, `MudDatePicker`), Attendees as a `MudChipSet` chip input (each chip a `MudChip`, 1 to 100 characters). Validation messages under each field. "Create" opens Meeting Detail. |
| Notes paste area | Meeting Detail | A multiline `MudTextField` with a 50,000 character limit and a live count. Caption beside the button: "Notes cannot be changed after saving". "Save notes" is enabled only when the textarea is non-empty and opens a confirm dialog with the same sentence. After save the textarea becomes read-only text rendered with `white-space: pre-wrap` and a caption "Saved {timestamp}, immutable". No edit affordance ever appears. |
| Run extraction button | Meeting Detail | When there are no Meeting Notes, the button is disabled with a visible caption "Add notes first" beneath it. On click it disables itself and shows an indeterminate `MudProgressLinear` with the caption "Extracting with {AI Provider} · {model}. This can take up to a minute with a local model." On success it navigates to the Review Screen for the new run. On failure it stays on Meeting Detail and shows the failed run in the list with its reason. The active AI Provider and model come from a read-only API endpoint (PRD FR-7). |
| Run list | Meeting Detail | A `MudTable`. Rows: started timestamp, Prompt Version, AI Provider and model, outcome, count of Proposed Actions, Pending count. Succeeded rows have "Review"; Failed rows have "Details". |
...
| Proposal card | Review Screen | See `DESIGN.md.Components`. Cards appear in AI return order and stay in place after a decision. A Pending card shows description, owner, and due date as read-only values with a "Proposed by AI" chip, the Source Excerpt blockquote, and three controls: Reject (text), Edit (outlined), Approve (filled). A null suggested due date reads "No due date proposed"; an empty suggested owner reads "Unassigned" with the hint "AI suggested: none". **Edit** switches the card into edit mode: description becomes a `MudTextField`, owner becomes a `MudSelect` of Users plus "Unassigned", due date becomes a `MudDatePicker` with a clear button. The owner select is pre-selected on a case-insensitive display-name match, and the hint "AI suggested: {text}" is always visible whether or not it matched. In edit mode the primary button reads "Approve with edits" while at least one field differs from the proposed value (the pre-selected owner counts as the proposed value) and reverts to "Approve" when nothing differs. A "Cancel" text button leaves edit mode and restores the proposed values. **Reject** opens a small dialog with an optional reason field and a "Reject" confirm. After any decision the card keeps its place and its Source Excerpt blockquote. The controls are replaced by the human provenance chip, the timestamp, and the Review State chip. A Rejected card shows "Reason: {text}" or "No reason given". An Edited card shows a "Proposed" column beside the decided values. An Approved or Edited card shows a "View action" link. → `mockups/review-screen.html`. |
...
Progress bars are `MudProgressLinear`, snackbars are raised through `ISnackbar`, and confirm dialogs are `MudDialog` instances opened through `IDialogService`.

| State | Surface | Treatment |
|---|---|---|
| Cold load | Every table and detail | `MudProgressLinear` under the toolbar; the body is empty until data arrives. No skeleton rows. |
...
Mouse-first with full keyboard operability. No custom shortcuts. `[ASSUMPTION: a three-day build does not budget for a shortcut layer; MudBlazor's native keyboard support is the floor.]`
...
- Tables use `MudTable` native semantics with sortable headers announced.
- Focus rings are MudBlazor defaults, visible on `surface` at AA.
...
| 1024 to 1199px | Notes pane becomes a `MudExpansionPanels` panel above the proposal cards, collapsed by default, expanded when a Source Excerpt is clicked. |
...
The Integrator is a system, not a person. This flow has no Blazor surface. The Blazor contribution is only step 2.

1. Dana approves a proposal on the Review Screen.
2. The decided card shows "Decided by Dana Whitfield". Nothing about the webhook is shown in the UI. `[ASSUMPTION: delivery state is visible through the API and the demo receiver page, not in the Blazor app, to keep P0 scope.]`
3. The demo receiver page, outside this spine, shows the signed event arriving.

Failure: the receiver is down. Retries and the Dead state are visible only through the API and the receiver page. Nothing changes in the Blazor app.
...
None blocking. Two `[ASSUMPTION]` tags in this spine (no shortcut layer; webhook state not in the Blazor app) and five in `DESIGN.md` (a MudBlazor base palette, purple/blue provenance hues, two-pane review, no navigation drawer, `Dense` tables) are defaults. The architect and story author may keep or reverse them without changing any FR.
