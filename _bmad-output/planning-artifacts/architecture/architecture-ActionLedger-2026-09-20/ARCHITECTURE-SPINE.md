...
graph TD
  ...
  Application --> Domain
  Web[src/ActionLedger.Web] -. generated client from committed openapi.json .-> Api
...
- **Binds:** Api, web, CI
- **Prevents:** hand-written client models drifting from the API; the web image needing a build toolchain at runtime; two builds disagreeing on the contract
- **Rule:** Routes are `/api/v1/<resource>` for resources `auth`, `users` (read-only `UserSummaryDto { id, displayName, role }` for any authenticated User), `meetings`, `meetings/<id>/notes`, `meetings/<id>/runs`, `runs/<id>`, `proposed-actions/<id>/decision`, `tracked-actions`, `tracked-actions/<id>/status`, `tracked-actions/<id>/revisions`, `webhook-subscriptions`, `outbox-messages` (read-only), `ai/provider`. Unversioned routes are only `/health` (liveness), `/health/ready` (checks the database), `/openapi`, and `/swagger`. `GET /api/v1/tracked-actions` accepts `ownerId=<uuid>|unassigned`, `status=<csv>`, `dueFrom`, `dueTo`, `meetingId`, `overdueOnly`, `sort=dueDate|status`, `dir`, `page`, `pageSize`. DTOs are defined in Application and are the only shapes that cross the boundary; the load-bearing fields are `ProposedActionDto { isLowConfidence, suggestedOwnerUserId, reviewState, decidedByUserId, decidedByDisplayName, decidedAt, rejectionReason, trackedActionId }`, `TrackedActionDto { proposedActionId, extractionRunId, meetingId, meetingTitle, promptVersion, ownerDisplayName, lastChangedByDisplayName, isOverdue }`, `ActionRevisionDto { actorDisplayName (null means AI), isLowConfidence on AiProposal }`, `MeetingSummaryDto { runCount, trackedActionCount }`, `RunSummaryDto { proposalCount, pendingCount }`. Errors are RFC 9457 `ProblemDetails` with `type` in {`validation`, `unauthorized`, `forbidden`, `not-found`, `conflict`}. Examples for extraction and review are attached with OpenAPI operation transformers; Swagger UI is served when `ASPNETCORE_ENVIRONMENT` is `Development` or `Compose`, and nginx proxies `/swagger` and `/openapi` as well as `/api`. `openapi.json` is committed at `src/ActionLedger.Web/openapi.json` (moved there by Story 1.5, when the Blazor project is created) and regenerated by `dotnet run --project src/ActionLedger.Api -- --export-openapi`; `Api.Tests/OpenApiSnapshotTest` generates the document through `WebApplicationFactory` and fails when it differs from the committed file. The web build generates its typed client with `NSwag.MSBuild` (MIT) into `src/ActionLedger.Web/Core/Api/` (git-ignored) as a pre-build target, so a contract change that has not been re-exported fails the Blazor build by compile error. Generation is kept rather than referencing the Application DTOs directly, even though both sides are C#: the committed document stays the boundary, so a server-side refactor cannot silently reshape the client.

### AD-14 — Frontend is component-per-route with feature folders and no HTTP in components

- **Binds:** web
- **Prevents:** components calling `HttpClient`; every feature fetching the user roster itself
- **Rule:** Feature folders `Auth`, `Meetings`, `Review`, `Actions`, `Audit` under `src/ActionLedger.Web/Features`. Each route has one routable container component; children are presentational, taking `[Parameter]` inputs and raising `EventCallback` outputs. Component classes hold state and command methods only. HTTP happens only in `Features/<Feature>/Data/<Feature>Service.cs`, which wraps the generated client. Cross-feature state is limited to the scoped services `Core/Auth/SessionState.cs` and `Core/Users/UserDirectory.cs` (loaded once after login). Enforced by an `Architecture.Tests` rule that fails the build when `System.Net.Http.HttpClient` or the generated client is referenced from any type outside `Core/` and `Features/*/Data/` — build-enforced rather than lint-enforced, so it cannot be skipped.
...
- **Rule:** `Domain.Tests` (pure, no mocks), `Application.Tests` (Fake provider, in-memory repository fakes), `Infrastructure.Tests` (Testcontainers PostgreSQL, real `AppDbContext`, the NFR-3 outbox transaction test, the inserts-only test, the revision-copy agreement test), `Api.Tests` (`WebApplicationFactory`, auth, ProblemDetails, `OpenApiSnapshotTest`, the concurrent-decision test), `Architecture.Tests` (AD-1), `Eval` (scorer against the fixture catalog), `Web.E2E` (one Playwright test of UJ-1 against compose with `Fake`). bUnit component tests live in `tests/Web.Tests`, run by `dotnet test` in `ci.yml`, and cover routable components and per-feature data services with the generated client mocked.
...
| Concern | Convention |
| --- | --- |
| Naming | Entities singular PascalCase; tables snake_case plural via a naming convention in `AppDbContext`; handlers `<Verb><Noun>Handler`; queries `<Feature>Queries`; ports `I<Noun>Repository`; domain events `<Noun><PastTenseVerb>`; DTOs `<Noun>Dto`, commands `<Verb><Noun>Command`. Blazor routable components `<Noun>Page.razor`, presentational components `<Noun>.razor`, per-feature clients `<Feature>Service.cs`, scoped state `<Noun>State.cs`. |
...
| API contract file | `src/ActionLedger.Web/openapi.json`, regenerated by `dotnet run --project src/ActionLedger.Api -- --export-openapi`, guarded by `OpenApiSnapshotTest`. |
| Pipeline | `ci.yml` on pull requests and `main`: restore, build, tests with Testcontainers, `Architecture.Tests`, `OpenApiSnapshotTest`, bUnit component tests, image build (the Blazor project builds and tests as part of `dotnet build` and `dotnet test`; there is no separate Node step). `eval.yml` on `prompts/**`, `fixtures/**`, `src/ActionLedger.Infrastructure/Ai/**`, `src/ActionLedger.Application/Ai/**`, `tests/Eval/**`, and manual dispatch. `cd.yml` on `main` and `v*`. Branch protection on `main` requires green `ci.yml` and a linked issue. |
...
| Microsoft.AspNetCore.Components.WebAssembly (MIT) | 10.0.12 |
| MudBlazor (MIT) | 9.10.0 |
| NSwag.MSBuild (MIT, generates the typed client from the committed contract) | 14.7.1 |
| bUnit (MIT, component tests) | 2.11.3 |
...
graph LR
  subgraph compose
    web[web: nginx + Blazor WASM] -->|/api, /swagger| api[api: ASP.NET Core + OutboxDispatcher + Seeder]
...
ActionLedger/
  ...
  src/
    ActionLedger.Web/      # Blazor WebAssembly
      Dockerfile openapi.json nginx.conf.template
      Core/                # SessionState, UserDirectory, delegating auth handler, generated api client (git-ignored)
      Features/{Auth,Meetings,Review,Actions,Audit}/   # <Noun>Page.razor + Data/<Feature>Service.cs
      Shared/              # ProvenanceChip, badges, status chips
  tools/
    webhook-receiver/    # echo service with Dockerfile; verifies HMAC in constant time
  tests/
    Domain.Tests/ Application.Tests/ Infrastructure.Tests/ Api.Tests/ Architecture.Tests/ Eval/ Web.Tests/ Web.E2E/
  docs/
